...
class ProductsFilter extends Model
{
    ...
    public static function getDynamicFilters($catIds){
        ...
        if(count($getFilterColumns)>0){
            ...
            $getFilterColumns = Product::whereIn('id',$getProductIds)->where('status',1)->get()->toArray();
        }
        $getFilterValues = array_filter(array_unique(Arr::flatten($getFilterValues)));
        // dd($getFilterValues);

        $getCategoryFilterColumns = ProductsFilter::select('filter_name')->whereIn('filter_value',$getFilterValues)->groupBy('filter_name')->orderBy('sort','ASC')->where('filter_value','!=',Null)->where('status',1)->pluck('filter_name')->toArray();  
