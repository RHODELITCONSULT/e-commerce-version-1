            ...
            if(!empty($data['courier_name'])&&!empty($data['tracking_number'])){
               Order::where('id',$data['order_id'])->update(['courier_name'=>$data['courier_name'],'tracking_number'=>$data['tracking_number']]); 
                ...
                Mail::send('emails.shipped_order',$messageData,function($message)use($email){
                    $message->to($email)->subject('Order Shipped (Tracking Details) - SiteMakers.in');
                });
            }
  ...
  <body>
    <header class="clearfix">
      ...
      <div id="company" class="clearfix">
        <div>Rhodel IT Consult</div>
        <div>SSNIT Emporium,<br /> Air Port City, Accra-Ghana</div>
        <div>(233) 244-4918-03</div>
        <div><a href="mailto:info@rhodelitconsult.com">rhodelitconsult.com</a></div>
      </div>
      <div id="project">
      ...
      <table>
        ...
        $output .='<tbody>
          <tr>
            ...
            <td class="qty">GHC '.$order['product_price'].'</td>
            <td class="qty">GHC '.$order['product_price']*$order['product_qty'].'</td>
          </tr>';
         }
         $output .='<tr>
            <td colspan="5">SUBTOTAL</td>
            <td class="total">GHC '.$total_price.'</td>
          </tr>
          <tr>
          ...
          <tr>
            <td colspan="5">DICOUNT</td>';
            if($orderDetails['coupon_amount']>0){
                $output .='<td class="total"> GHC '.$orderDetails['coupon_amount'].'';
            }else{
                $output .='<td class="total">GHC 0</td>';
            }
             $output .='</tr>
          <tr>
            <td colspan="5" class="grand total">GRAND TOTAL</td>
            <td class="grand total">GHC '.$orderDetails['grand_total'].'</td>
          </tr>
        </tbody>
      </table>
      <div id="notices">
